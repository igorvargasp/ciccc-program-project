...
import { Swords, History, Lock } from 'lucide-react';
...
export default function Simulator() {
  ...
  const [resultStandings, setResultStandings] = useState<StandingRow[] | null>(null);
  ...
  const simulate = useMutation({
    ...
    onSuccess: (result) => {
      setResultStandings((result.resultingStandings as StandingRow[]) ?? null);
      qc.invalidateQueries({ queryKey: ['simulations'] });
    },
  });

  const selectedMatch = scheduledMatches?.find((m) => m.id === selectedMatchId);
  const homeTeam = selectedMatch ? teamsMap.get(selectedMatch.homeTeamId) : undefined;
  const awayTeam = selectedMatch ? teamsMap.get(selectedMatch.awayTeamId) : undefined;

  if (!token) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4 text-center">
        <Lock className="w-12 h-12 text-muted opacity-40" />
        <h2 className="text-xl font-extrabold text-foreground">{t('simulator.title')}</h2>
  ...
  return (
    <div className="space-y-8 max-w-3xl">
      ...
      {/* Match selector */}
      <div className="bg-surface border border-edge/12 rounded-2xl p-5 space-y-4">
        <h2 className="text-base font-extrabold text-foreground">{t('simulator.selectMatch')}</h2>
          ...
          {scheduledMatches?.map((m) => {
            ...
            return (
              <option key={m.id} value={m.id}>
                {home?.shortName ?? '?'} vs {away?.shortName ?? '?'} — {formatMatchDay(m.kickoffAt)} {formatKickoff(m.kickoffAt)}
              </option>
            );
          })}
        </select>
        ...
        {selectedMatch && (
          <div className="flex items-center justify-between gap-4 bg-surface-2 rounded-xl p-4">
            ...
            <div className="flex flex-col items-center gap-1.5 flex-1">
              {homeTeam?.crestUrl && (
                <img src={homeTeam.crestUrl} alt={homeTeam.name} className="w-10 h-10 object-contain" />
              )}
            ...
            <div className="flex flex-col items-center gap-1.5 flex-1">
              {awayTeam?.crestUrl && (
                <img src={awayTeam.crestUrl} alt={awayTeam.name} className="w-10 h-10 object-contain" />
              )}
        ...
        <Button
          ...
          onClick={() => simulate.mutate({ matchId: selectedMatchId, homeScore, awayScore })}
          className="w-full"
        >
          <Swords className="w-4 h-4" />
          {simulate.isPending ? t('simulator.loading') : t('simulator.simulate')}
        </Button>

        {simulate.isError && (
          <p className="text-sm text-red-400 text-center">{t('errors.generic')}</p>
        )}
      </div>
      ...
      {resultStandings && resultStandings.length > 0 && (
        <div className="bg-surface border border-edge/12 rounded-2xl p-5">
          <h2 className="text-base font-extrabold text-foreground mb-4">{t('simulator.projectedStandings')}</h2>
      ...
      <div>
        ...
        {loadingSims ? <PageSpinner /> : !simulations?.length ? (
                ...
                <div
                  ...
                  onClick={() => {
                    if (sim.resultingStandings) {
                      setResultStandings(sim.resultingStandings as StandingRow[]);
                    }
                  }}
                >
