...
export default function PlayerCard({ player, compact }: PlayerCardProps) {
  ...
  return (
    <Link to={`/players/${player.id}`}>
      <div
        className={cn(
          "group bg-surface border border-edge/12 rounded-xl hover:border-brand/35 transition-all duration-150 flex items-center gap-3",
          compact ? "p-3" : "p-4",
        )}
      >
        ...
        <div className="flex-shrink-0 w-11 h-11 rounded-full overflow-hidden bg-surface-2 border border-edge/20 flex items-center justify-center">
          {player.photoUrl ? (
            ...
            <span className="text-sm font-black text-muted group-hover:text-foreground transition-colors">
              #{player.shirtNumber}
            </span>
          ) : (
        ...
        <div className="min-w-0 flex-1">
          <p className="font-bold text-foreground group-hover:text-brand transition-colors truncate text-sm">
            {player.fullName}
          </p>
